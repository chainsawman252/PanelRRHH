    ...
    <div className="container" style={{ padding: 24 }}>
      <div className="dashboard-top">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <div>
            <h2 className="dashboard-title">Panel RRHH</h2>
            <div className="dashboard-sub">{companyName ? `Empresa: ${companyName}` : 'Filtrando por empresa'}</div>
          </div>
          <div className="dashboard-controls">
            <label className="small-muted">Días:</label>
            ...
            <button className="btn" onClick={() => load(days)}>Actualizar</button>
          </div>
        </div>

        <div ref={mapDivRef} className="map-hero" />
      </div>

      <div className="dashboard-bottom" style={{ marginTop: 20 }}>
        <section className="dashboard-card fichajes-section">
          ...
          <table className="fichajes-table">
            ...
            <tbody className="fichajes-row">
              {rows.map(r => (
                <tr key={r.id}>
                  <td style={{ display: 'flex', alignItems: 'center' }}>
                    <span className="fichaje-avatar">{(r.usuario?.nombre || r.usuario?.email || '').charAt(0).toUpperCase()}</span>
                    <div>
                      <div style={{ fontWeight: 700 }}>{r.usuario?.nombre || r.usuario?.email}</div>
                      <div className="small-muted">{r.usuario?.email}</div>
                    </div>
                  </td>
        ...
        <div className="indicators-graphs">
          <section className="dashboard-card">
            <h3 style={{ marginTop: 0 }}>Indicadores</h3>
            <div style={{ display: 'flex', gap: 12 }}>
              <div className="card" style={{ padding: 12, flex: 1 }}>Total fichajes: <strong>{rows.length}</strong></div>
              <div className="card" style={{ padding: 12, flex: 1 }}>Entradas hoy: <strong>{rows.filter(r => r.tipo === 'IN').length}</strong></div>
              <div className="card" style={{ padding: 12, flex: 1 }}>Salidas hoy: <strong>{rows.filter(r => r.tipo === 'OUT').length}</strong></div>
            </div>
          </section>

          <section className="dashboard-card">
            <h3 style={{ marginTop: 0 }}>Gráficos</h3>
            <div style={{ minHeight: 220 }} className="small-muted">Aquí irán las gráficas — se mostrarán abajo del mapa como solicitaste.</div>
          </section>
        </div>
      </div>
