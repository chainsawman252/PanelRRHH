  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const needsAdminSetup =
        data.session.user.user_metadata?.created_via === 'admin-panel' &&
        !data.session.user.user_metadata?.admin_setup_completed;

      if (needsAdminSetup) {
        const { data: finalizeData, error: finalizeErr } = await supabase.rpc('finalizar_admin_setup');
        console.log('finalizar_admin_setup result', { finalizeData, finalizeErr });
        if (finalizeErr) {
          throw finalizeErr;
        }
      }
    ...
    <div className={styles.authPage}>
      <div className={styles.authCard} role="main" aria-labelledby="login-title">
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, marginBottom: 8 }}>
          {/* logo inside green box */}
          <div className={styles.logoBox} aria-hidden={true}>
            <img src={kronosLogo} alt="Kronos" className={styles.logoImg} />
          </div>
